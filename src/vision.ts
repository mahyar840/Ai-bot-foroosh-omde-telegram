...
export async function rewriteCaption(
  ...
  const prompt =
    "متن زیر یک کپشن محصول از یک تامین‌کننده/فروشنده عمده است. باید یک نسخه‌ی تمیزشده ازش بسازی. " +
    "قانون اصلی: تمام خط‌ها و بولت‌هایی که درباره‌ی ویژگی، مشخصات فنی، کیفیت یا کاربرد محصول هستن رو کامل و بدون خلاصه‌سازی نگه دار — " +
    "همه‌شون رو، نه فقط یکی دوتا. فقط این موارد رو حذف کن:\n" +
    ...
    "همه‌ی خط‌های دیگه (اسم محصول، برند، مدل، و تک‌تک ویژگی‌های فنی) رو دقیقاً با همون تعداد خط نگه دار، فقط لحنش رو کمی حرفه‌ای‌تر کن. " +
    "خیلی مهم: این کار خلاصه‌نویسی نیست — فقط پاک‌سازی قیمت و اطلاعات تماسه. طول خروجی باید تقریباً هم‌اندازه‌ی متن اصلی (منهای خط‌های قیمت و تماس) باشه. " +
    "خروجی هرگز نباید خالی یا خیلی کوتاه‌تر از متن اصلی باشه. " +
